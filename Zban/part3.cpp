...
vector<double> f(vector<pair<string, double> > v) {
    ...
    for (auto o : v) {
        ...
        assert(id < S || o.first == "");
        res[find(substs, substs + S, o.first) - substs] = o.second;
    }
    return res;
}
...
void solve(vector<vector<double> > a, double delta, double T) {
    ...
    a.push_back(f({{H2, 1.0}, {N2, 1.0}, {"", 98740}}));

    vector<double> res_x_g;
    vector<double> res_x;
    for (double x_g = 0.1; x_g < 0.95; x_g += 0.1) {
        ...
        vector<double> cx(6, 0.0);
        for (int i = 0; i < 5; i++) cx[i] = P_g[variables[i]] + 1;
        cx[5] = 0.5;
        int iters = 1;
        print(cx);
        for (;; iters++) {
        ...
        cerr << "x^g " << x_g << " simple iterations finished with " << iters << " iterations" << endl;
        double x = cx[5];
